...
def draw_callback_tooth_select(self, context):
    ...
    for i in range(0,len(tooth_button_data)): #each of those is a loop        
        ...
        for j in range(0,len(self.rest_lists)):
            if tooth_button_names[i] in self.rest_lists[j]:
...
def draw_callback_gl_raycast_thickness(self, context):  
    ...
    def execute(self,context):
        for i in range (0,4):
            for tooth_name in self.rest_lists[i]:
                bpy.ops.view3d.append_working_tooth('EXEC_DEFAULT',name = str(tooth_name), rest_type = str(i))
        for tooth_name in self.rest_lists[4]:
            bpy.ops.view3d.append_working_space('EXEC_DEFAULT',name = str(tooth_name))  
...
class PonticDesign(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if tooth.rest_type != '1': #safer long term.  if tooth.rest_types[int(tooth.rest_type)] != 'PONTIC'
        ...
        if self.p_types[int(self.p_type)] == 'OVATE':
            ...
            current_objects=list(bpy.data.objects)                
            bpy.ops.mesh.primitive_uv_sphere_add(location = tuple(ov_loc))
            for o in bpy.data.objects:
               if o not in current_objects:
        ...
        if self.p_types[int(self.p_type)] == 'TISSUE':
            ...
            mod.use_project_z=True
            mod.offset=self.offset #perhaps negative
            mod.target= Tissue
...
class RaycastThickness2Obj(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        if context.area.type == 'VIEW_3D':
            ...
            # Add the region OpenGL drawing callback
            # draw in view space with 'POST_VIEW' and 'PRE_VIEW'
            self._handle = context.region.callback_add(draw_callback_gl_raycast_thickness, (self, context), 'POST_PIXEL')
            self.mouse =(0,0)
            return {'RUNNING_MODAL'}
