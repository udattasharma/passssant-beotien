...
color_background = bpy.context.user_preferences.themes['Default'].view_3d.space.back
d['color_background'] = list(color_background) + [1.0]	# alpha
...
class Data():
	...
	offset_3d = Vector() # ou utiliser local_grid_origin, center_ofs,  pour l'origine des coordonnées 3d
	view = 'top'
...
#			in_region = False
	...
	def set_scale_draw_options(self):
		...
		vec = self.offset_3d * self.viewmat
...
#					break
			if not(started):
				...
				_handle = bpy.context.region.callback_add(draw_callback_px,
											   (self, context),
											   'POST_PIXEL') # 'POST_VIEW', 'POST_PIXEL', 'PRE_VIEW'
